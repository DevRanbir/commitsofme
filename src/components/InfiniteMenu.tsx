...
interface Camera {
    ...
    aspect: number;
    position: vec3;
    width?: number; // Added to fix type error if used
    height?: number; // Added to fix type error if used
    up: vec3;
    matrices: {
        view: mat4;
        projection: mat4;
...
class InfiniteGridMenu {
    ...
    private init(onInit?: InitCallback): void {
        ...
        // Only subdivide if we have enough items to fill the larger sphere (42 vertices vs 12)
        // This prevents excessive repetition of items when specific projects count is low
        if (this.items.length > 12) {
            this.icoGeo.subdivide(1);
        }
    ...
    private initTexture(): void {
        ...
        const cellSize = 1024; // Increased from 512 for better quality
        ...
        Promise.all(
                ...
                item =>
                    new Promise<HTMLImageElement>(resolve => {
                        ...
                        img.onerror = () => {
                            ...
                            fallback.onerror = () => {
                                // Last resort: a 1x1 base64 PURPLE pixel that is definitely safe and visible
                                const safeFallback = new Image();
                                safeFallback.src = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8/x8AAwMCAO+ip1sAAAAASUVORK5CYII=";
                                safeFallback.onload = () => resolve(safeFallback);
                            };
                            fallback.src = 'https://picsum.photos/1024/1024?grayscale'; // Increased fallback resolution
                        };
                        img.src = item.image;
                    })
            )
            ...
            images.forEach((img, i) => {
                ...
                // Object-fit: cover implementation to prevent squashing
                const aspect = img.width / img.height;
                let sx = 0, sy = 0, sWidth = img.width, sHeight = img.height;

                if (aspect > 1) {
                    // Landscape: crop to Center square
                    sWidth = img.height;
                    sx = (img.width - sWidth) / 2;
                } else {
                    // Portrait: crop to Center square
                    sHeight = img.width;
    ...
    private initDiscInstances(count: number): void {
        ...
        const loc = this.discLocations.aInstanceMatrix;
        // mat4 takes 4 vec4 attributes
        for (let i = 0; i < 4; ++i) {
        ...
        <div style={{ position: 'relative', width: '100%', height: '100%', minHeight: '600px' }}>
            ...
            {activeItem && items.length > 0 && (
                <div className="absolute inset-0 pointer-events-none flex flex-col items-center justify-center">
                    {/* Center the content roughly */}
                    <div className="relative w-full h-full">
                        {/* Position text based on user CSS logic but simpler for React */}
                        {/* Show text only when not moving to trigger reveal effect nicely */
                            !isMoving && (
                                <>
                                    <BlockRevealText key={`${activeItem.title}-title`} className="face-title text-foreground font-black">
                                        {activeItem.title}
                                    </BlockRevealText>

                                    <LineRevealText key={`${activeItem.title}-desc`} justify="center" className="face-description text-muted-foreground bg-background/80 p-2 rounded-md">
                        ...
                        <button onClick={handleButtonClick} className={`action-button ${isMoving ? 'inactive' : 'active'} pointer-events-auto z-50`}>
